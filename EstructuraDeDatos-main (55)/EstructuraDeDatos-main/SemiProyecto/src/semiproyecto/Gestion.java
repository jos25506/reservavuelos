...
public class Gestion {
    ...
    public Cliente[] cargarInfo() {
        ...
        for (int i = 0; i < pojoListaAt.length; i++) {
            ...
            try {
                String[] partesHoraCreacion = aHoraCreacion.split(":");
                String[] partesHoraAtencion = aHoraAtencion.split(":");

                int segundosCreacion = Integer.parseInt(partesHoraCreacion[0]) * 3600
                        + Integer.parseInt(partesHoraCreacion[1]) * 60
                        + Integer.parseInt(partesHoraCreacion[2]);

                int segundosAtencion = Integer.parseInt(partesHoraAtencion[0]) * 3600
                        + Integer.parseInt(partesHoraAtencion[1]) * 60
                        + Integer.parseInt(partesHoraAtencion[2]);

                double tiempoAtencion = Math.max(0, segundosAtencion - segundosCreacion);
                int indiceCaja = i % 3; // va a distribuir a los clientes entre Caja 0, 1 y 2

                ContadorReportes.registrarAtencion(indiceCaja, tiempoAtencion);
            } catch (Exception e) {
                System.out.println("Error al calcular tiempo de atención: " + e.getMessage());
            }
        }
...
public Cliente[] cargarAtendidos(String configuracion) {
        ...
        for (int i = 0; i < pojoListaAt.length; i++) {
            ...
            // Calcular tiempo de atención para clientes
            try {
                String[] partesHoraCreacion = a.getHoraCreacion().split(":");
                String[] partesHoraAtencion = a.getHoraAtencion().split(":");
                ...
                // se toma el tiempo de atención en el contador global
                int indiceCaja = 0; 
                ContadorReportes.registrarAtencion(indiceCaja, tiempoAtencion);
            } catch (Exception e) {
                System.out.println("Error al calcular tiempo en atención: " + e.getMessage());
            }
        }
